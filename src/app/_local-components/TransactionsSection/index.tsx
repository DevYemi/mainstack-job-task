...
import { Button, ButtonGroup, SkeletonCircle, SkeletonText, Stack, chakra } from '@chakra-ui/react';
    ...
    return (
        <chakra.section mt={"8.2rem"}>
            <chakra.div display={"flex"} justifyContent={"space-between"} gap={"2.4rem"}>
                <SkeletonText speed={1} spacing={4} noOfLines={2} skeletonHeight={"2rem"} isLoaded={!isLoading}>
            ...
            <chakra.div display={"flex"} flexDirection={"column"} gap={"2.4rem"}>
                {
                    isLoading ?
                        new Array(7).fill(null).map(() => (
                            <Stack key={nanoid()} alignItems={"center"} spacing={"1.45rem"} direction={"row"}>
                                <SkeletonCircle speed={1} size={"4.8rem"} />
                                <SkeletonText speed={1} flex={1} spacing={3} skeletonHeight={"1.6rem"} noOfLines={2} />
                            </Stack>
                        ))
                        :
                        data?.map(item => (
                            <chakra.div key={nanoid()} display={"flex"} gap={"1.45rem"} alignItems={"center"}>
