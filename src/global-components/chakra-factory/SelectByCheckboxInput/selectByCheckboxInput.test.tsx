...
const getExpectedValueArr = (arr: Array<any>) => {
    return arr.filter(item => item?.defaultValue)
        .map(item => item?.value)
        .join(", ")
}
describe("SelectByCheckboxInput Unit Tests", () => {
    ...
    test("Should render properly without errors", () => {
        ...
        expect(screen.getByTestId("selectByCheckboxInputWrapper")).toBeInTheDocument()
    });
    ...
    test("Should on first render set ReadOnlyInput value to checklist item defaultValue set to true", async () => {
        ...
        const readOnlyinput = screen.getByTestId<HTMLInputElement>("selectByCheckboxInputReadOnlyInput");
